...
namespace combi4  //размещения
{
	...
	xcombination::xcombination(short n, short m) {
		...
		this->m = m;					// количество элементов в размещении 
		this->sset = new short[m + 2];	// массив индесов текущего размещения
		this->reset();
	}
	...
	short accomodation::getnext() {
		short rc;
		this->na++;							// номер размещения 0, ..., count()-1
